...
La home è divisa per **materia**, un solo livello di menu in più rispetto a prima (il pulsante 🏠 riporta sempre dritto alla home principale, mai al menu della materia — comportamento identico ovunque nell'app):

- **🧮 Matematica**
  - **🔢 Numeri** — da 1 a 10, con il numero grande, gli oggetti da contare e la voce che li pronuncia.
  - **🔷 Forme e Figure** — un disegno fatto di forme geometriche colorate (cerchio giallo, triangolo blu, rettangolo verde, quadrato rosso), si conta quante ce ne sono di un certo tipo+colore e si scrive il numero con una vera **tastiera numerica** (0-9, ⌫, ✓) — l'unica attività dell'app che la usa, invece della scelta a tocco, per scrivere davvero il numero come su un foglio.
  ...
  - **✍️ Scrivi i Numeri (dettato)** — la voce detta un numero e il bambino lo scrive col dito sulla lavagna. Se non riesce, aiuti progressivi: compare la traccia grigia da ricalcare (poi più scura) e il controllo diventa via via più generoso. Non conta dove e quanto grande si scrive: conta la forma.
- **📚 Italiano**
  - **🔤 Lettere** — l'alfabeto italiano (21 lettere), ognuna con una parola e un'immagine ("A come Ape 🐝"). Nei momenti di insegnamento si sente il **suono** della lettera ("mm", non "emme"), come si insegna oggi a scuola — più naturale da collegare alla lettura, pronunciato molto lentamente per essere ben percepibile. Contiene anche il gioco "🧩 Completa la parola": una parola con una lettera nascosta, si ascolta la parola e si sceglie la lettera mancante.
  ...
  - **✍️ Scrivi le Lettere (dettato)** — come sopra, ma per le lettere: la voce pronuncia il suono e il bambino lo scrive col dito.
